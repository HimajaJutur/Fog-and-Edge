...
def is_valid_reading(temp, humidity, airflow, cpu_load) -> bool:
    ...
    if not (1 <= humidity <= 100):
        return False
    if not (0 <= airflow <= 100):
        return False
    if not (0 <= cpu_load <= 100):
        return False
    return True
...
# ─────────────────────────────────────────
def generate_sensor_data(scenario: str = "random"):
    """
    ...
    # Occasionally inject a faulty spike to test the anomaly filter
    if random.random() < 0.05:
        temp = 0

    if not is_valid_reading(temp, humidity, airflow, cpu_load):
        print("[SENSOR] Faulty reading detected — discarded by anomaly filter.")
        return None
...
def run(scenario: str, interval: int):
    print(f"[SENSOR] Starting simulator  →  {FOG_NODE_URL}")
    ...
    while True:
        ...
        try:
            ...
            forwarded = result.get("cloud_forwarded", False)
            print(f"[SENSOR] Fog Node → STATUS: {status}  |  Cloud forwarded: {forwarded}\n")
        except requests.exceptions.ConnectionError:
            print("[SENSOR] Could not reach Fog Node — is fog_node.py running?\n")
